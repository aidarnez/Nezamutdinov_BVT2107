...
class MainWindow(qtw.QWidget):
    ...
    def keypad(self):
        ...
        btn_point = qtw.QPushButton('.', clicked = lambda: self.func_press('.'))
        ...
        container.layout().addWidget(btn_minus,5,3)
    ...
    def func_press(self, operator):
        temp_string = ''.join(self.temp_nums)
        if operator == '.' and (self.last_operator[-1] == '.' or self.last_operator[-1] == ''):
            operator = ''
        self.last_operator.append(operator)
    ...
    def func_result(self):
        ...
        if '/0' in str(fin_string):
            self.result_field.setText('Нельзя делить на ноль!')
        else:
